let adminChannel=null,currentApplication=null,lastPendingTotal=-1;
...
async function loadStats(){
  ...
  if(ca.error||ra.error||c.error||r.error||o.error){toast("Some dashboard data could not be loaded");}
...
function appCard(type,x){
  ...
  return `<div class="admin-card"><b>${esc(title)}</b><small>${esc(sub)}</small><small>${esc(area)}</small><div class="card-actions"><button class="orange" onclick="openApplication('${type}','${x.id}')">VIEW</button><button class="green" onclick="approveApplication('${type}','${x.id}')">APPROVE</button><button class="decline" onclick="rejectApp('${type}','${x.id}')">REJECT</button></div></div>`;
}
async function loadApplications(){
  ...
  if($("newApplicationText"))$("newApplicationText").textContent=total?`${total} application${total===1?"":"s"} waiting for your review.`:"";
...
function openApplication(type,id){
  fetchApplication(type,id).then(a=>{
    ...
    const rows=type==="cook"?[
      ["Full name",a.full_name],["Display / kitchen name",a.display_name],["Phone",a.phone],["Email",a.email||"Not provided"],
      ["Food type",a.food_type],["Service area",a.service_area],["Postal codes",a.service_postal_codes],
      ["Operating hours",`${a.operating_start||"--"} – ${a.operating_end||"--"}`],["Daily capacity",a.daily_capacity],
      ["SFA licensed",a.sfa_licensed?"Yes":"No"],["SFA licence number",a.sfa_licence_number||"Not provided"],["Bio",a.bio||"--"],
      ["Compliance acknowledgement",a.compliance_ack?"Accepted":"Not accepted"]
    ]:[
      ["Full name",a.full_name],["Phone",a.phone],["Email",a.email||"Not provided"],["Vehicle",a.vehicle_type],
      ["Operating area",a.operating_area],["Compliance acknowledgement",a.compliance_ack?"Accepted":"Not accepted"],
...
async function approveApplication(type,id){
  ...
  if(type==="cook"){
    const {data,e}=await supabase.from("merchants").insert({
      name:a.display_name||a.full_name,type:a.food_type||"Local Food",bio:a.bio||"",phone:a.phone||null,
      service_area:a.service_area||null,service_postal_codes:a.service_postal_codes||null,
      sfa_licensed:a.sfa_licensed||false,sfa_licence_type:a.sfa_licence_type||null,
      sfa_licence_number:a.sfa_licence_number||null,sfa_licence_expiry:a.sfa_licence_expiry||null,
      operating_start:a.operating_start,operating_end:a.operating_end,
      order_open:a.operating_start,order_close:a.operating_end,daily_capacity:a.daily_capacity||20,
      status:"approved",active:true,menu_live:true
    }).select("id").single();
    createdId=data?.id;createError=e;
  }else{
    const {data,e}=await supabase.from("riders").insert({
      name:a.full_name,phone:a.phone||null,vehicle_type:a.vehicle_type||null,
      operating_area:a.operating_area||null,status:"approved",active:true,
      compliance_ack:a.compliance_ack||false,compliance_ack_version:a.compliance_ack_version||null,
      compliance_ack_at:a.compliance_ack_at||null,eligibility_ack:a.eligibility_ack||false,
      eligibility_ack_at:a.eligibility_ack_at||null
    }).select("id").single();
    createdId=data?.id;createError=e;
  }
  ...
  // Use the schema's existing merchant_id/rider_id fields; do not rely on a non-existent column.
  const updatePatch=type==="cook"
    ?{status:"approved",approved_at:now,merchant_id:createdId}
    :{status:"approved",approved_at:now,rider_id:createdId};
  const {error:updateError}=await supabase.from(table).update(updatePatch).eq("id",id);
  if(updateError)return toast(updateError.message);

  // Store a partner-facing approval message when the notification migration is installed.
  const recipientUserId=a.user_id||null;
  try{
    const {error:notificationError}=await supabase.from("partner_notifications").insert({
      application_id:id,partner_type:type,recipient_user_id:recipientUserId,recipient_email:a.email||null,
    ...
    if(notificationError&&!String(notificationError.message||"").toLowerCase().includes("relation"))toast("Approved, but notification could not be saved");
  }catch(_){/* notification migration is optional during staging */}
...
function subscribe(){
  ...
  adminChannel=supabase.channel("management-live")
    ...
    .on("postgres_changes",{event:"*",schema:"public",table:"merchants"},refreshAll)
    .on("postgres_changes",{event:"*",schema:"public",table:"riders"},refreshAll)
    .subscribe();
}

document.addEventListener("DOMContentLoaded",async()=>{
  if(!kbAuthReady()){go("login");toast("Database not connected");return;}
  ...
  if(isOAuthCallback&&user){
    ...
    // Always show the management login on a fresh page load.
    go("login");
  }
  supabase.auth.onAuthStateChange((event,session)=>{
    if(event==="SIGNED_IN"&&session?.user){
      const callback=new URLSearchParams(window.location.search).get("auth")==="callback";
      if(callback){window.history.replaceState({},document.title,window.location.pathname);enterDashboard(session.user);}
    }
    if(event==="SIGNED_OUT"){lastPendingTotal=-1;go("login");}
  });
});
